...
How sessions are _run_, as distinct from what they build. `docs/next-features.md` was the
...
assert the _wrong_ answer on purpose. Three touch every file in the repo. One is a decision
...
in the graph. Mutual exclusion is _not_ an edge — it is a mutex, and it belongs in a
...
`blocks` constrains ordering. `unblocks` constrains _value_ and is what tells you which
...
The tell is always the same: a file that is an _interface between_ work items rather than
...
Gates go where a wrong answer is expensive _and_ cannot be detected automatically. If a
...
An allowlist communicates through its _omissions_, so state them: there is no `Bash(rm *)`,
...
5. **Anything reaches MCP's stdout.** stdout _is_ the JSON-RPC transport (invariant 2).
...
but because the task has not been specified well enough for _anything_ to verify it, and
...
_wrong_: you get a merge conflict in the best case and a silent last-writer-wins in the
...
A background run that _writes code_ returns a diff nobody watched being made, which
...
- **G5 is Opus despite being a constants change** — it crosses four packages (core, cli,
  ...
  `LOCK_TIMEOUT_MS` is a _different_ concept and must not be folded in.
- **G2 is a judgment node.** Its comment in `findings.md` argues MCP should probably
  _queue_ rather than reject, unlike HTTP — an agent should not settle that alone.
...
That list is short because it was _checked_, not asserted: `grep -l` for the symbols each
...
_Why first:_ it is the layer you are already standing on. Everything in Part B is
...
- _Claude Code best practices_, Anthropic Engineering —
...
_Why second:_ Part B's graph is hand-drawn ASCII. This track is about what changes when the
graph becomes a runtime object — with state, checkpoints, and resumption.

- _Building effective agents_, Anthropic Engineering —
  <https://www.anthropic.com/engineering/building-effective-agents>
  — the workflows-vs-agents distinction underpinning this whole document. Read it first.
- _How we built our multi-agent research system_, Anthropic Engineering —
...
_Why third:_ once the graph can pause and resume, _where_ to pause becomes the design
...
_Why last:_ it is the track that tells you whether any of the previous three worked. It
...
**Exercise.** None to build — one to _recognise_. `qa/` is already an eval harness: a
...
Write down which of the 210 assertions have _never_ been seen red. Those are the ones with
...
`--check` looks for the _shape_ of K9 — two nodes editing one path with neither
...
B3 closes by saying its register is short because it was _checked_, and that a
...
G11, G14 and G1 are all withheld. A8's own rule already says this — _parallel
lanes do not pay when two nodes touch the same file; that is not slower, it is
wrong_ — it was simply not visible until the paths were data.
...
_draft_ the decision; a human signs off by creating the file.
...
This is what makes B4's deliberate step — run the suite _before_ touching the
...
- **Hooks guard the agent's tools, not its subprocesses.** `guard.mjs` sees
  `Write`, `Edit` and `Bash` calls. A script the agent _runs_ — including
